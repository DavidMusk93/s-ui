...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/DavidMusk93/s-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/DavidMusk93/s-ui/main/install.sh"
...
update_shell() {
    wget -O /usr/bin/s-ui -N --no-check-certificate https://github.com/DavidMusk93/s-ui/raw/main/s-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
